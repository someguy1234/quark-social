...
static const char* SERVER = "chat.freenode.net";
...
IrcClient::IrcClient(QWidget* parent) : QSplitter(parent)
{
    ...
    nickAction->setStatusTip(tr("Use the new nickname set in the options menu"));
    nickAction->setToolTip(nickAction->statusTip());
    nickAction->setCheckable(false);
    ...
    textEdit->append(IrcMessageFormatter::formatMessage(tr("! Welcome to the Quark IRC %1 client.").arg(IRC_VERSION_STR)));
   // textEdit->append(IrcMessageFormatter::formatMessage(tr("! This client connects to %1 and joins %2.").arg(SERVER, CHANNEL)));
...
void IrcClient::createToolBars()
{
    QToolBar* toolbarirc = new QToolBar(tr("Social toolbar"));
    //toolbar->setOrientation(Qt::Vertical);
    toolbarirc->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
    toolbarirc->addAction(nickAction);
    QWidget* view = new QWidget();
    //QWidget* view = new QTextEdit();

    QVBoxLayout* vbox = new QVBoxLayout();
    vbox->addWidget(toolbarirc);
    vbox->addWidget(view);
...
void IrcClient::nicknameChange()
{
    ...
    connection->setUserName("QuarkFan");
//    connection->setNickName(tr("QuarkFan%1").arg(qrand() % 9999));
    ...
    if (nick.isEmpty())
        nick = tr("QuarkFan%1").arg(qrand() % 9999);

    od.setNick(nick);

    connection->setNickName(nick);
    connection->setRealName(tr("%1 client").arg(IRC_VERSION_STR));
}

void IrcClient::onTextEditChanged()
{
    // Autoscroll chat
    QTextCursor c =  textEdit->textCursor();
    c.movePosition(QTextCursor::End);
    textEdit->setTextCursor(c);
    textEdit->ensureCursorVisible(); // you might need this also
}
...
{
    setWindowTitle(tr("Quark IRC %1 client").arg(IRC_VERSION_STR));
...
void IrcClient::createParser()
{
    ...
    parser->addCommand(IrcCommand::Message, "MSG (<channel/user>) <message...>");
